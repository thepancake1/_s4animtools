...
from _s4animtools.asm.states import StateProperties, LIST_OT_NewState, LIST_OT_DeleteState, LIST_OT_MoveState, \
    ...
    PostureProperties, PosturePanel, LIST_OT_NewPosture, LIST_OT_DeletePosture, LIST_OT_MovePosture, StateConnections, LIST_OT_NewStateConnection, LIST_OT_DeleteStateConnection,\
LIST_OT_MoveStateConnection
...
from _s4animtools.clip_processing.value_types import float32, uint32
from _s4animtools.clip_processing.clip_header import ClipResource
from _s4animtools.ik_baker import s4animtool_OT_bakeik


from _s4animtools.control_rig.basic_control_rig import CopyLeftSideAnimationToRightSideSim
from _s4animtools.ik_manager import BeginIKMarker, LIST_OT_NewIKTarget, LIST_OT_DeleteIKTarget, LIST_OT_MoveIKTarget, \
    s4animtool_OT_removeIK, s4animtool_OT_mute_ik, s4animtool_OT_unmute_ik, LIST_OT_NewIKRange, LIST_OT_DeleteIKRange, LIST_OT_DeleteSpecificIKTarget
...
# TODO Script has way too any responsibilities!!!
...
def determine_ik_slot_targets(rig):
    ...
    What this does is for each Copy Transformation
    constraint, it assumes it's an IK constraint. It also assumes
    the object the Copy Transformation constraint is pointing to is
    another Empty object that actually points to the bone the sim should
    animate relative to.

    Now also falls back to assigning bone.name to all constraints if it's not found in the default sim rigs. See rig constants
    Returns a list of bones that should be IK'ed.
    """
    all_constraints = defaultdict(list)
    for bone in rig.pose.bones:
        for con in bone.constraints:
            if isinstance(con, bpy.types.CopyTransformsConstraint):
                if con.name.startswith("IKFollow"):
                    try:
                        all_constraints[grips_mapping[bone.name.lower()]].append(
                            (con.target.constraints[0].target, con.target.constraints[0].subtarget, con))
                    except:

                        all_constraints[bone.name].append(
                            (con.target.constraints[0].target, con.target.constraints[0].subtarget, con))
                        pass
...
def gather_ik_weights(influences, constraint, start_frame, current_frame, last_frame_influence):
   ...
   # print(last_frame_influence - constraint.influence)
    if current_frame - start_frame == 0 or abs(last_frame_influence - constraint.influence) > 0.001:
        influences[constraint][current_frame-start_frame] = constraint.influence
...
class ClipExporter(bpy.types.Operator):
    ...
    def setup_events(self, context, current_clip, start_frame, frame_count, additional_snap_frames):
        ...
        variable_to_event = {context.object.parent_events_list: ParentEvent, context.object.sound_events_list: SoundEvent,
                             context.object.snap_events_list: SnapEvent, context.object.visibility_events_list : VisibilityEvent,
                             context.object.script_events_list : ScriptEvent, context.object.reaction_events_list : ReactionEvent,
                             context.object.play_effect_events_list : PlayEffectEvent, context.object.focus_compatibility_events_list:FocusCompatibilityEvent,
        ...
        for parameter_fields, event in variable_to_event.items():
            for event_instance in parameter_fields:
                ...
                if parameter_length < event.arg_count:
                    raise Exception(f"Your event has incomplete parameters. Expected {event.arg_count} parameters. Got {parameter_length}")
                try:
                    ...
                    if event == FocusCompatibilityEvent:
                        if frame_time >= timeshifted_timestamp >= 0:
                            _, timeshifted_end_timestamp = self.create_timeshifted_timestamp(parameters[1].strip(), start_time)
                            current_clip.add_event(event(timeshifted_timestamp, timeshifted_end_timestamp, *parameters[2:]))
    ...
    def execute(self, context):
        ...
        constraints = determine_ik_slot_targets(bpy.context.active_object)
        weight_channels = []
        ...
        def setup_channel_data(bone):
            ...
            default_previous_frame = {
                    "Main Channel": {"TRANSLATION": Vector((0, 0, 0)), "ORIENTATION": Quaternion((1, 0, 0, 0)),  "SCALE": Vector((1,1,1))},
        ...
        for bone in constraints:
            for idx, target in enumerate(constraints[bone]):
                slot_assignments[bone].append((bone, target[0], target[1], idx, int(target[2].target.name.split(" ")[2])))
        ...
        def recursive_bone_animate(bone, idx, is_snap_frame):
            if bone.parent and keyframed_bones[bone.name] and bpy.context.scene.is_overlay or not bpy.context.scene.is_overlay and bone.parent:
                        ...
                        if bone.name == bone_constraint_name:
                            for bone_constraint in slot_assignments[bone_constraint_name]:
                                ...
                                animate_relative_to_bone(bone, bone_constraint[1].pose.bones[bone_constraint[2]],
                                                         bpy.context.active_object, bone_constraint[1], idx, channel_name, is_snap_frame)
            for child in bone.children:
                # Don't animate slots.
        ...
        def animate_relative_to_bone(bone, parent_bone, source_rig, target_rig, idx, channel_name, is_snap_frame, allow_fail_on_scale=True):
        ...
        for clip_idx in range(len(clip_indices)-1):
            ...
            current_clip = ClipResource(combined_clip_name, rig_name, slot_assignments,
                                        ...
                                        reference_namespace_hash, initial_offset_q,
                                        initial_offset_t, blend_name, has_loco_bone, context.object.disable_rig_suffix)
            setup_channel_data(root_bone)


            clip_start, clip_end = clip_frame_idx, clip_indices[clip_idx + 1]

            snap_frames = self.setup_events(context, current_clip, clip_start, clip_end - clip_start, context.object.additional_snap_frames)
            ...
            for current_frame in range(clip_start, clip_end):
                ...
                for bone in constraints:
                    for idx, target in enumerate(constraints[bone]):
                        ik_weight = gather_ik_weights(ik_weight_animation_data, target[2], clip_start, current_frame, last_frame_influences[target[2]])
                        last_frame_influences[target[2]] = ik_weight
            for bone in constraints:
                for idx, target in enumerate(constraints[bone]):
                    current_clip.clip_body.add_channel((create_ik_weight_channels(bone, ik_weight_animation_data[target[2]], idx + 1)))
...
class NewClipExporter(bpy.types.Operator):
    ...
    def execute(self, context):
        current_clip = ClipResource("a2o_test", context.object.rig_name, [],
                                    context.object.explicit_namespaces,
                                    context.object.reference_namespace_hash, "0,0,0,1",
                                    "0,0,0", "a2o_test", False)
        rig = context.object
        exporter = AnimationExporter(rig, rig)
        exporter.create_animation_data()
        bpy.context.scene.frame_set(bpy.context.scene.frame_start)
        exporter.animate_recursively(bpy.context.scene.frame_start, force=True)

        for idx in range(bpy.context.scene.frame_start+1, bpy.context.scene.frame_end - 1):
            bpy.context.scene.frame_set(idx)
            exporter.animate_recursively(idx)

        bpy.context.scene.frame_set(bpy.context.scene.frame_end)


        exporter.animate_recursively(bpy.context.scene.frame_end, force=True)


        for channel in exporter.export_to_channels():
            current_clip.clip_body.add_channel(new_channel=channel)

        current_clip.clip_body.set_clip_length(bpy.context.scene.frame_end - bpy.context.scene.frame_start)

        current_clip.update_duration(bpy.context.scene.frame_end - bpy.context.scene.frame_start)
        current_clip.serialize()
        print(exporter)
        return {"FINISHED"}
...
class S4ANIMTOOLS_PT_MainPanel(bpy.types.Panel):
    ...
    def draw(self, context):
       # self.layout.operator("s4animtools.copy_baked_animation", icon='MESH_CUBE', text="Copy Baked Animation")
        self.layout.operator("s4animtools.copy_baked_animation_new", icon='MESH_CUBE', text="Copy Baked Animation New")
       ...
       # self.layout.operator("s4animtools.copy_left_side_sim_selected", icon='MESH_CUBE', text="Copy Left Side (Sim) Selected")
        self.layout.operator("s4animtools.maintain_keyframe", icon="MESH_CUBE", text="Maintain Keyframe").direction = "FORWARDS"
        self.layout.operator("s4animtools.maintain_keyframe", icon="MESH_CUBE", text="Maintain Keyframe Backward").direction = "BACK"
...
#
        ...
        if obj is not None:
            ...
            self.draw_events(obj, "parent_events_list", 0.1, "Parameters (Frame Number/Object To Be Parented/Object To Be Parented To/Bone)", "Parent Events", self.layout)
            self.draw_events(obj, "sound_events_list", 0.1, "Parameters (Frame Number/Sound Effect Name)", "Sound Events",  self.layout)
            self.draw_events(obj, "script_events_list", 0.1, "Parameters (Frame Number/Script Xevt)", "Script Events",  self.layout)
            self.draw_events(obj, "snap_events_list", 0.1, "Parameters (Frame Number/Actor/Translation/Quaternion)", "Snap Events",  self.layout)
            self.draw_events(obj, "reaction_events_list", 0.1, "Parameters (Frame Number/Reaction ASM)", "Reaction Events",  self.layout)
            self.draw_events(obj, "play_effect_events_list", 0.1, "Parameters (Frame Number/Actor Hash/Bone Name Hash/(always 0)/(almost always 0)/Unique VFX Name)", "Play Effect Events",  self.layout)
            self.draw_events(obj, "stop_effect_events_list", 0.1, "Parameters (Frame Number/Unique VFX Name/(always 0)/Unknown Bool 1)", "Stop Effect Events",  self.layout)
            self.draw_events(obj, "disable_lipsync_events_list", 0.1, "Parameters (Frame Number/Duration)", "Suppress Lipsync Events",  self.layout)
            self.draw_events(obj, "visibility_events_list", 0.1, "Parameters (Frame Number/Actor/Visibility)", "Visibility Events",  self.layout)
            self.draw_events(obj, "focus_compatibility_events_list", 0.1, "Parameters (End Frame,Level)", "Focus Compatibility Events",  self.layout)
            ...
            if obj.ik_idx >= 0 and obj.ik_targets:
                row = self.draw_all_ik_targets_of_type(context, obj, row, "b__L_Hand__")
                row = self.draw_all_ik_targets_of_type(context, obj, row, "b__R_Hand__")
                row = self.draw_all_ik_targets_of_type(context, obj, row, "b__L_Foot__")
                row = self.draw_all_ik_targets_of_type(context, obj, row, "b__R_Foot__")
                row = self.draw_all_ik_targets_of_type(context, obj, row, "b__ROOT_bind__")
                row = self.draw_all_ik_targets_of_type(context, obj, row, "")
    ...
    def draw_all_ik_targets_of_type(self, context, obj, row, chain_bone):
        excluded = ["b__ROOT_bind__", "b__L_Hand__", "b__R_Hand__", "b__L_Foot__", "b__R_Foot__"]
        for idx, item in enumerate(obj.ik_targets):
            if chain_bone == "":
                if item.chain_bone not in excluded:
                    row = self.draw_ik_target(context, idx, item, obj, row)

            elif item.chain_bone == chain_bone:
                row = self.draw_ik_target(context, idx, item, obj, row)
        return row
    def draw_ik_target(self, context, idx, item, obj, row):
        row = row.column()
        sub = row.row()
       # if idx == obj.ik_idx:
        ...
        sub.label(text="IK Target")
        sub.scale_x = 0.2
        sub.operator('iktarget.delete_specific', text='Delete').command = str(idx)
        sub.operator('iktarget.new', text='Clone').command = f"{item.chain_bone},{item.holder_src_bone}"

        sub = row.row(align=True)
        sub.prop_search(item, "chain_bone", obj.pose, "bones")
        sub.prop_search(item, "holder_src_bone", obj.pose, "bones")
        sub = row.row(align=True)
        try:
        ...
        sub.prop(item, "chain_idx", text = "Chain Idx")
      #  sub = row.row(align=True)
        # sub = row.row(align=True)
      #  sub.prop(item, "sequence_idx", text="Sequence Idx")
        for range_idx, range in enumerate(item.ranges):
            sub = row.row(align=True)

            sub.prop(range, "start_time", text="Start Time")
            sub.prop(range, "end_time", text="End Time")
            sub.operator('iktarget.delete_ik_range', text="Delete").command = f"{idx},{range_idx}"
        # for range in item.ranges:
        #     sub.prop(range, "start_time", text="Start Time")
        #    sub.prop(range, "end_time", text="End Time")
        row.operator('iktarget.new_ik_range', text="New Range").command = str(idx)
        return row
...
class OffsetCalculator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        bpy.context.active_object.initial_offset_q = ",".join([str(round(rotation[1], 4)), str(round(rotation[2], 4)), str(round(rotation[3], 4)), str(round(rotation[0], 4))])
        bpy.context.active_object.initial_offset_t = ",".join([str(round(translation[0], 4)), str(round(translation[1], 4)), str(round(translation[2], 4))])
...
class ExportAnimationStateMachine(bpy.types.Operator):
    ...
    def create_actor(self, actor_name, actor_type, is_master, is_virtual):
        ...
        return actor_status
    def create_posture_manifest(self, actor_name, posture_name, posture_family, compatibility, carry_left, carry_right, surface):
        if posture_family == "":
            posture_family = "none"
    ...
    def create_state(self, name, type, skippable, focus, facial_overlay, controllers, interrupt_this):
        ...
        for controller in controllers:
            text+=self.create_controller(controller.name, controller.target, controller.focus, controller.mask, controller.track, controller.blendin, controller.blendout)
        text += "</State>"
        return text
    ...
    def execute(self, context):
        ...
        anim_path = os.path.join(os.environ["HOMEPATH"], "Desktop", "Animation Workspace", "02D5DF13!00000000!" + get_64bithash(context.object.name.lower()) +"." + context.object.name +  ".AnimationStateMachine")
        ...
        for actor in context.object.postures:
            text +=  self.create_posture_manifest(actor.actor1, actor.posture_name, actor.posture_family, actor.compatibility, actor.carry_left, actor.carry_left, actor.surface)
        for state in context.object.states:
            text += self.create_state_header(state.name, state.public, state.skippable, state.focus, state.facial_overlays, state.controllers, state.interrupt_this)
        ...
        for state in context.object.states:
            text += self.create_state(state.name, state.public, state.skippable, state.focus, state.facial_overlays, state.controllers, state.interrupt_this)
        text += "</ASM>"
...
classes = (
    ...
    TimeRange,
    IKTarget, s4animtool_PT_IKTargetPanel, LIST_OT_NewIKTarget, LIST_OT_DeleteIKTarget, LIST_OT_MoveIKTarget,
    s4animtool_OT_bakeik, s4animtool_OT_removeIK,
    BeginIKMarker, s4animtool_OT_unmute_ik, s4animtool_OT_mute_ik, NewClipExporter, PositionConfig, QuaternionConfig, ActorSettings, ClipData, ImportRig,
  CopyLeftSideAnimationToRightSideSim,
    LIST_OT_MoveActor, ActorPanel, LIST_OT_NewState, LIST_OT_DeleteState, LIST_OT_MoveState, StatePanel, ActorProperties, ControllerProperties, StateProperties,
LIST_OT_NewActor, LIST_OT_DeleteActor, LIST_OT_NewController, LIST_OT_RemoveController, LIST_OT_MoveControllerState, PosturePanel, PostureProperties,
LIST_OT_NewPosture,LIST_OT_DeletePosture,LIST_OT_MovePosture, OffsetCalculator, StateConnections, LIST_OT_NewStateConnection, LIST_OT_DeleteStateConnection,
LIST_OT_MoveStateConnection, ExportAnimationStateMachine, MaintainKeyframe, AnimationEvent, InitializeEvents, S4ANIMTOOLS_OT_move_new_element,AnimationEvent,
...
def register():
    ...
    bpy.types.Object.l_hand_fist = FloatProperty(default=0, soft_min=0, soft_max=1)
